...
if __name__ == '__main__':
    ...
    mask = np.load('./singlegoal_dataset/mask.npy')
    mask = torch.tensor(mask, dtype=torch.float)
    dist_in = np.load('./singlegoal_dataset/dist_in.npy')
    dist_in = torch.tensor(dist_in[:Ntotal, :, :], dtype=torch.float)
    input = smooth_chi(mask, dist_in, smooth_coef)
    output = np.load('./singlegoal_dataset/output.npy')
    output = torch.tensor(output, dtype=torch.float)

    goals = np.load('./singlegoal_dataset/goals.npy')
