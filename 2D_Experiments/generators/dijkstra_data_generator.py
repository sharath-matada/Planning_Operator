        ...
        for goal_trial in range(goal_trials):
            ...
            if passable_indices.size > 0:
                ...
                while np.all(travel_time_values_array[trial * goal_trials + goal_trial, :, :] == 0):
                    ...
                    env = Environment2D(goal,1-velocity_matrix)
                    valueDijkstra = AStar.getDistances(env)
                    ...
                    travel_time_values_array[trial * goal_trials + goal_trial, :, :] = valueDijkstra
                    high_values_mask = valueDijkstra > 10e5
                ...
                if np.any(travel_time_values_array[trial * goal_trials + goal_trial, :, :] > 10e4):
                    print("Improper Data")
                    break
        ...
        for trial in tqdm(range(num_trials)):
            ...
            for goal_trial in range(goal_trials):
                ...
                if passable_indices.size > 0:
                    ...
                    environment = 1-ndimage.binary_erosion(1-np.array(original_maze),iterations=np.random.randint(1,20)).astype(np.array(original_maze).dtype)
                    while np.all(travel_time_values_array[trial * goal_trials + goal_trial + offset, :, :] == 0):
                        ...
                        travel_time_values_array[trial * goal_trials + goal_trial+offset, :, :] = valueDijkstra
                        high_values_mask = valueDijkstra > 10e5
                        input_mask = (velocity_matrix == 0)
                    ...
                    if np.any(travel_time_values_array[trial * goal_trials + goal_trial + offset, :, :] > 10e4):
