...
class MahalanobisMetric(nn.Module):
    def __init__(self, num_features, size):
        ...
        if isinstance(size, tuple):
            self.layers = nn.ModuleList([nn.Linear(num_features, s, bias=True) for s in size])
        else:
            self.layers = nn.ModuleList([nn.Linear(num_features, size, bias=True)])

    def forward(self, x, y):
        if len(self.layers) > 1:
            out = torch.cat([layer(x - y) for layer in self.layers], dim=-1)
        else:
            out = self.layers[0](x - y)
